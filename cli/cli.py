...
def cmd_proof(args):
    ...
    if verified is True:
        print(f"Verified: [PASS] Lean 编译器验证通过")
    elif verified is False:
        verification = result.get("verification") or {}
        print(f"Verified: [FAIL] Lean 编译未通过")
        print(f"  {verification.get('output', '')[:400]}")
    else:
        toolchain = result.get("toolchain") or {}
        print(f"Verified: [SKIP] {toolchain.get('message', 'Lean 工具链缺失')}")
    print(f"\nProof steps:")
...
def cmd_lean_doctor(args):
    """检测 Lean 工具链（lean/lake/mathlib）"""
    from core.formal_proof_engine import check_toolchain
    status = check_toolchain()
    print("Lean toolchain status:")
    print(f"  lean  : {status['lean'] or 'MISSING'}")
    print(f"  lake  : {status['lake'] or 'MISSING'}")
    print(f"  mathlib: {'available' if status['mathlib'] else 'not fetched'}")
...
def main():
    ...
    # proof
    p_proof = sub.add_parser("proof", help="生成形式化证明（真编译验证）")
    p_proof.add_argument("theorem", help="定理名")
    p_proof.add_argument("--style", default="verbose", choices=["verbose", "lean_only"])

    # lean-doctor
    sub.add_parser("lean-doctor", help="检测 Lean 工具链（lean/lake/mathlib）")
