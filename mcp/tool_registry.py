...
class ToolRegistry:
            ...
            self._tools.append({
                ...
                "inputSchema": {
                    ...
                    "properties": {
                        ...
                        "seed": {"type": "integer", "description": "随机种子, 默认723003"},
                    },
                },
            })
    ...
    async def _handle_verify_symbolic(self, params: dict) -> str:
        """Tool 8: SymPy 符号一致性验证（真实执行模型推导步骤）"""
        model_name = params.get("model", "network_embedded_growth")
        if model_name not in self._models:
            return json.dumps({"error": f"Model '{model_name}' not found", "status": "FAIL"}, ensure_ascii=False)
        verifier = VerificationEngine(title=f"SymPy Symbolic Verification: {model_name}")
        engine = SymbolicEngine()
        instance = self._models[model_name]()

        steps = instance.get_derivation_steps()
        if not steps:
            verifier.add_symbolic_check(
                "no-steps", lambda: False, detail="Model defines no derivation steps"
            )
        for step in steps:
            ...
            if method is None:
                verifier.add_symbolic_check(
                    method_name, lambda: False, detail=f"Method {method_name} missing"
                )
                continue

            def run_step(_m=method, _n=method_name):
                try:
                    output = _m(engine, {})
                    # 步骤应返回 dict 且不抛异常；含 verified=True 视为通过
            ...
            verifier.add_symbolic_check(
                method_name,
                run_step,
                detail=f"Executes derivation step '{method_name}' with a fresh SymbolicEngine",
            )
        report = verifier.get_report()
    ...
    async def _handle_network_metrics(self, params: dict) -> str:
        ...
        return json.dumps({
            "pipeline": "专利引用网络度量计算",
            "computed_metrics": {
                "XE_XF": "同行业/跨行业引证数 (平均/总和/中位数)",
                "pagerank": "PageRank中心性 (α=0.85, max_iter=100)",
                "betweenness": "介数中心性 (采样k=500)",
                "clustering": "聚类系数 (局部方法)",
                "constraint": "结构洞约束 (Burt, 1992)",
                "community": "社区检测 (贪心模块度)",
                "degree": "度中心性 (入度+出度)",
            },
            "regression_specs": {
                "M1": "DV ~ XE + Controls",
                "M2": "DV ~ XF + Controls",
                "M3": "DV ~ XE + XF + Controls",
                "M4": "DV ~ XE + XE² + XF + XF² + Controls (核心二次型)",
                "M5_log": "DV ~ ln(XE) + ln(XF) + Controls",
            },
    ...
    async def _handle_multi_agent_verify(self, params: dict) -> str:
        """QED 多Agent对抗验证（真实 FOC/边界检验 + 可选 LLM 角色）"""
        import numpy as np
        ...
        # 引擎 Proposer：二次型 LPG = α₁·x + α₂·x² 的解析推导
        def proposer_fn(c):
            return {
                "proof": "FOC: d/dx (α₁·x + α₂·x²) = α₁ + 2α₂·x = 0 → x* = -α₁/(2α₂). "
                         "SOC: d²/dx² = 2α₂ determines U (α₂>0) or inverted-U (α₂<0).",
                "confidence": 0.9,
                "turning_point_formula": "x* = -a1/(2*a2)",
                "parameters": {"a1": 1.0, "a2": -1.0},
            }

        # 真实 FOC 检验：数值中心差分验证 f'(x*) ≈ 0
        def lpg(params_dict):
            a1 = params_dict.get("a1", 1.0)
            a2 = params_dict.get("a2", -1.0)
            x = params_dict.get("_x", 0.0)
            return a1 * x + a2 * x**2

        def grad_lpg(params_dict):
            a1 = params_dict.get("a1", 1.0)
            a2 = params_dict.get("a2", -1.0)
            x = params_dict.get("_x", 0.0)
            return {"_x": a1 + 2 * a2 * x}
        ...
        def turning_point(params_dict):
            a1 = params_dict.get("a1", 1.0)
            a2 = params_dict.get("a2", -1.0)
            return -a1 / (2 * a2) if abs(a2) > 1e-9 else float("nan")

        foc_critic = MultiAgentVerificationEngine.make_foc_critic(
            lpg, grad_lpg, param_gen, turning_point, n_samples=n_samples, tolerance=tolerance
        )
        critic_fns = [foc_critic]

        # 真实边界检验（a2 → ±边界时 x* 行为）
        edge_critic = MultiAgentVerificationEngine.make_edge_case_critic(
            {"a1": (-3, 3), "a2": (-3, 3)}, n_samples=max(50, n_samples // 4)
        )
        # 注入真实 edge_check：x* 有限 且 |a2| 不小到数值病态
        def edge_check(params_dict):
            tp = turning_point(params_dict)
            if not np.isfinite(tp):
                return True, f"non-finite turning point at params={params_dict}"
            if abs(params_dict["a2"]) < 1e-6:
                return True, f"degenerate a2≈0 at {params_dict}"
            return False, ""

        critic_fns.append(edge_critic)

        # 可选 LLM 角色（环境配置 API key 后启用；未配置自动降级为引擎模式）
        model_client = None
        if use_model:
            try:
    ...
    async def _handle_unified_verify_pipeline(self, params: dict) -> str:
        ...
        if "symbolic" in levels:
            ...
            if model_name in self._models:
                ...
                for step in instance.get_derivation_steps():
                    ...
                    def run_step(_m=method, _n=step["method_name"]):
                        try:
                            output = _m(engine, {})
                    ...
                    verifier.add_symbolic_check(
                        step["method_name"], run_step,
                        detail=f"Executes '{step['method_name']}' on a fresh SymbolicEngine",
                    )
            else:
                verifier.add_symbolic_check("model-missing", lambda: False, detail=f"Model {model_name} not found")
            report = verifier.get_report()
            pipeline_results["levels"]["symbolic"] = {
                "status": "PASS" if report.pass_rate() >= 100 else "FAIL",
        ...
        if "formal_proof" in levels:
            ...
            verified = proof_result.get("verified")
            if verified is True:
                status = "PASS"
                note = f"Lean 编译器验证通过 (exit 0)"
            elif verified is False:
                status = "FAIL"
                note = f"Lean 编译未通过: {(verification or {}).get('output', '')[:300]}"
            else:
                status = "SKIP"
                note = proof_result.get("toolchain", {}).get("message", "Lean 工具链缺失")
            pipeline_results["levels"]["formal_proof"] = {
                "status": status,
                "theorem": proof_result.get("statement", ""),
                "lean_code_available": "lean_code" in proof_result,
                "verified": verified,
                "verification": verification,
                "note": note,
            }

        # Level 5: QED 多Agent对抗验证（真实 FOC 检验 + 可选模型）
        if "multi_agent" in levels:
            ma_engine = MultiAgentVerificationEngine()

            def proposer(c):
                return {
                    "proof": f"FOC: dLPG/dXE = α₁+2α₂·XE = 0 → XE* = -α₁/(2α₂). SOC: d²LPG/dXE² = 2α₂.",
                    "confidence": 0.92,
                }
            ...
            def glpg(pd):
                a1, a2 = pd.get("a1", 1.0), pd.get("a2", -1.0)
                return {"_x": a1 + 2 * a2 * pd.get("_x", 0.0)}
            ...
            critic_fns = [MultiAgentVerificationEngine.make_foc_critic(lpg, glpg, pgen, tpf, n_samples=200, tolerance=1e-4)]
            model_client = None
            try:
                from core.llm_client import get_client as _get_llm_client
                candidate = _get_llm_client()
    ...
    async def _handle_analyze_ode(self, params: dict) -> str:
        ...
        # Parse the ODE expression (安全解析，替代 eval)
        # 1) 拦截危险 token：禁止属性访问链 / 导入 / 系统调用
        _banned = ["__", "import", "exec", "eval", "open(", "system(", "subprocess",
                   "getattr", "globals", "locals", "vars(", "compile("]
        if any(_token in expr_str for _token in _banned):
            return json.dumps({
                "error": "expression contains disallowed tokens (__ / import / open / system / getattr / ...)",
                "status": "error",
            }, ensure_ascii=False)
        # 2) 用 sympy.parse_expr 在受限命名空间解析（本地符号 + 常用函数），不执行任意代码
        _local = {
            "x": x, "f": f, "Eq": sp.Eq, "diff": sp.diff,
            "exp": sp.exp, "sin": sp.sin, "cos": sp.cos,
            "log": sp.log, "Symbol": sp.Symbol, "Function": sp.Function,
        }
        try:
            ode = sp.parse_expr(expr_str, local_dict=_local, global_dict={}, transformations="all")
        except Exception:
            ode = sp.diff(f, x) - f * x
